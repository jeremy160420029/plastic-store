                                ...
                                <td>
                                    <div class="btn-group">
                                        <form method="POST" action="{{ route('transaksi.accept', ['transactionId' => $t->id]) }}">
                                            @csrf
                                            <button type="submit" class="btn btn-success custom-btn">Accept</button>
                                        </form>

                                        <form method="POST" action="{{ route('transaksi.decline', ['transactionId' => $t->id]) }}">
                                            @csrf
                                            <button type="submit" class="btn btn-danger custom-btn">Decline</button>
                                        </form>

                                        <a href="#" class="btn btn-success custom-btn" data-bs-toggle="modal" onclick="showTransaction({{ $t->id }})" data-bs-target="#modal-transaction">Detail</a>

                                        <a class="btn btn-danger custom-btn" onclick="modalDeleteTrans({{ $t->id }})"><i class="ti ti-trash"></i></a>
                                    </div>
                                </td>
                            </tr>
                            @endforeach
